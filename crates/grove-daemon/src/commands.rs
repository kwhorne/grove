...
fn build_env_snippet(
    ...
    let is_db = |s: &&&grove_services::ServiceStatus| {
        s.key == "mysql" || s.key == "postgres" || s.key == "elyrasql"
    };
    let db = installed
        .iter()
        .find(|s| s.running && is_db(s))
        .or_else(|| installed.iter().find(is_db));

    if let Some(db) = db {
        // ElyraSQL speaks MySQL's protocol, so Laravel's `mysql` driver is the
        // right one; it has exactly one database, so that name goes in too.
        let conn = if db.key == "postgres" {
            "pgsql"
        } else {
            "mysql"
        };
        let db_name = if db.key == "elyrasql" {
            grove_services::ELYRASQL_DATABASE.to_string()
        } else {
            db_name.clone()
        };
        out.push_str(&format!(
